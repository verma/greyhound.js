...
var Stats = function(obj) {
    ...
    var walkNode = function(root, k) {
        return k.split("/").reduce(function(acc, v) {
            if (!acc) return acc;
            return acc[v];
        }, root);
    };
    ...
    o._as = function(v) {
        ...
        var isLeafLike = util.has(node, "value") && util.has(node, "type");
...
BBox.prototype.deflate = function(by) {
    by = util.isArray(by) ? by.map(function(a) { return -a; }) : -by;
    return this.inflate(by);
};

BBox.prototype.offsetBy = function(by) {
    if (!util.isArray(by) || by.length !== 3)
